...
import org.perses.delta.WeightSplitDeltaDebugger
...
class WeightedPristineHDDReducer(
  ...
  override fun internalReduce(fixpointReductionState: FixpointReductionState) {
    internalReduceWDD(fixpointReductionState)
  }

  private fun internalReduceWDD(fixpointReductionState: FixpointReductionState) {
    ...
    while (currentLevel.isNotEmpty()) {
      val debugger = WeightSplitDeltaDebugger(
        createDeltaArguments(
          needToTestEmpty = true,
          tree,
          actionsDescription = "[pristine-weighted-hdd]",
          input = currentLevel,
        ),
        FanoutWeightProvider()
      )
