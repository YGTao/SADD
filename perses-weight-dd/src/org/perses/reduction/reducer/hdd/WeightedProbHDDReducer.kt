...
import org.perses.delta.PristineProbabilisticDeltaDebugger
import org.perses.delta.WeightedPristineProbabilisticDeltaDebugger
import org.perses.delta.FanoutWeightProvider
...
class WeightedProbHDDReducer(
  ...
  override fun internalReduce(fixpointReductionState: FixpointReductionState) {
    internalReducePDD(fixpointReductionState)
  }

  private fun internalReducePDD(fixpointReductionState: FixpointReductionState) {
    ...
    while (currentLevel.isNotEmpty()) {
      val debugger = WeightedPristineProbabilisticDeltaDebugger(
        createDeltaArguments(
          needToTestEmpty = true,
          tree,
          actionsDescription = "[weighted-ProbDD]",
          input = currentLevel,
        ),
        weightProvider = FanoutWeightProvider() 
      )
  ...
  companion object {
    ...
    const val NAME = "weighted_prob_hdd"
